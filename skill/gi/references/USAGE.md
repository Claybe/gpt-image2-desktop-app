...
`generate-image` 是一个 Claude Code skill，用于通过兼容 OpenAI Images API 的接口生成图片。它提供两个入口：`/gi-setup` 初始化 API 配置，`/gi` 生成图片。默认模型为 `gpt-image-2`（兼容旧配置里的 `gpt-image2` / `auto`，helper 会映射为 `gpt-image-2`），默认出图比例为 `custom`（从提示词获取；未获取到则使用 API `auto`），默认清晰度为 `1k`，默认输出目录为项目目录下的 `.claybe/.generate-image/`。
...
| resolution | 否 | 清晰度档位：`1k`、`2k`、`4k`；默认 `1k` |
...
| resolution | No | Resolution tier: `1k`, `2k`, or `4k`; defaults to `1k` |
