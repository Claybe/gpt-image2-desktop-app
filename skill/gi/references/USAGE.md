...
`generate-image` 是一个 Claude Code skill，用于通过兼容 OpenAI Images API 的接口生成图片。它提供两个入口：`/gi-setup` 初始化 API 配置，`/gi` 生成图片。默认模型为 `gpt-image-2`（兼容旧配置里的 `gpt-image2` / `auto`，helper 会映射为 `gpt-image-2`），默认出图比例为 `custom`（从提示词获取；未获取到则使用 API `auto`），默认尺寸为 `auto`；Agent 应根据资产用途、性能成本与视觉细节需求选择具体宽高。默认输出目录为项目目录下的 `.claybe/.generate-image/`。
...
```bash
npx --yes skills add Claybe/gpt-image2-desktop-app --skill gi --agent claude-code --global --copy --yes
npx --yes skills add Claybe/gpt-image2-desktop-app --skill gi-setup --agent claude-code --global --copy --yes
```

安装到当前项目：

```bash
npx --yes skills add Claybe/gpt-image2-desktop-app --skill gi --agent claude-code --copy --yes
npx --yes skills add Claybe/gpt-image2-desktop-app --skill gi-setup --agent claude-code --copy --yes
```
...
| --- | --- | --- |
| prompt | 是 | 图片提示词 |
| aspect-ratio | 否 | 出图比例：`custom`、`16:9`、`9:16`、`3:2`、`4:3`、`1:1`；默认 `custom`，会从提示词获取比例，未获取到则使用 API `auto` |
| size | 否 | 输出尺寸：`auto` 或 `<宽>x<高>`；Agent 根据资产用途、性能成本与视觉细节需求选择具体宽高，无法判断时使用 `auto` | `auto` |
...
`gpt-image*` 模型默认先尝试流式生成。如果流式响应没有最终完成图但已经返回草稿/中间图，helper 会使用最后一张可用图片；如果流式响应完全没有图片或接口拒绝 stream，helper 会自动回退一次普通非流式生成。生成请求本身不支持从中断处续传，网络或代理中断后只能重新发起生成；返回 URL 后的图片下载也依赖服务端是否支持 Range，当前 helper 不做断点续传。
...
`generate-image` is a Claude Code skill for generating images through an OpenAI Images API-compatible endpoint. It provides two entries: `/gi-setup` configures API settings, and `/gi` generates images. The default model is `gpt-image-2` (legacy `gpt-image2` / `auto` config values are mapped to `gpt-image-2` by the helper), the default aspect ratio is `custom` (extract from the prompt; if none is found, API `auto` is used), the default size is `auto` with the agent choosing concrete dimensions from asset purpose, performance cost, and visual-detail needs, and the default output directory is `.claybe/.generate-image/` under the project directory.
...
| aspect-ratio | No | Aspect ratio: `custom`, `16:9`, `9:16`, `3:2`, `4:3`, or `1:1`; defaults to `custom`, which extracts the ratio from the prompt and falls back to API `auto` if none is found |
| size | No | Output size: `auto` or `<width>x<height>`; the agent chooses concrete dimensions based on asset purpose, performance cost, and visual-detail needs, using `auto` when uncertain | `auto` |
...
`gpt-image*` models try streaming first by default. If the stream has no final completed image but already returned draft/intermediate image data, the helper uses the latest available image; if the stream contains no image at all or the endpoint rejects streaming, the helper automatically retries once without streaming. The generation request itself cannot resume from an interruption and must be started again; after a URL is returned, image download resumption depends on server-side Range support, and the current helper does not implement resumable downloads.
