---
name: gi-setup
description: Use when the user wants to configure the /gi image generation endpoint, save URL/API key/model settings, or read them from setting.json. This is the setup command for /gi and should guide missing URL/key values step by step instead of attempting image generation.
argument-hint: "[url=<api-base-url>] [apikey=<api-key>] [model=gpt-image-2] [--use-settings] [--settings <path>]"
---
